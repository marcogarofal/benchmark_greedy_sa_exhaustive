...
Usage:
    ...
    # Global seed limit (applies to ALL plot types)
    python3 plot_multi_algorithm.py --all --max-seeds 400
    python3 plot_multi_algorithm.py --performance --max-seeds 200
...
def plot_quality_50pct_metrics(max_seeds=None, save_path=None):
    ...
    NOTE: quality statistics are pre-computed in checkpoints, so max_seeds
    affects only the statistics that were computed at benchmark time.
    This function filters raw data where available, otherwise uses stored stats.
    """
    checkpoints = get_all_checkpoints()
    ...
    if max_seeds is not None:
        print(f"ℹ️  Note: quality metrics are pre-computed in checkpoints; "
              f"--max-seeds={max_seeds} does not re-filter quality stats.")
    ...
    if max_seeds is not None:
        title += f'\n(max_seeds={max_seeds} — note: quality stats pre-computed)'
    fig.suptitle(title, fontsize=16, fontweight='bold')
...
def plot_exhaustive_only(max_nodes=None, max_seeds=None, save_path=None):
    ...
    for i, ratio in enumerate(ratios):
        ...
        offset = (i - 1) * width
        ax.bar(x + offset, means, width, label=f'Discretionary {ratio}',
               color=colors[ratio], yerr=stds, capsize=5,
               error_kw={'linewidth': 2})
        
        for j, (mean, std) in enumerate(zip(means, stds)):
            if mean > 0:
                label = f'{mean*1000:.2f}ms' if mean < 0.01 else \
...
def parse_args():
    """Parse --max-seeds and --max-nodes from sys.argv. Returns (max_nodes, max_seeds)."""
...
def main():
    ...
    # Build filename suffix for filters
    suffix = ''
    if max_seeds is not None:
